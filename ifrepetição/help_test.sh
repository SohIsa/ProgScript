...
if [ $1 = logica ]; then 
	echo "Multiple Expressions can be combined using the following operators, listed in decreasing order of precedence.
...
elif [ $1 = aritmetica ]; then
	echo "These are normally used in conjunction with another non-math test operator somewhere in the expression.
...
ARG1 -ge ARG2   Returns true if ARG1 is greater-than-or-equal to ARG2
	...
	echo "Comparison strings for test do not need to be quoted, though you can quote them to protect characters with special meaning to the shell, e.g. spaces.
...
As of bash 4.1 (2010), string comparisons made with [[ and using < or > will respect the current locale."
	...
	echo "These options test for particular types of files. All cases will only return True (0) if the file exists.
